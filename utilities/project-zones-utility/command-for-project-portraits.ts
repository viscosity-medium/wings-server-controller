...
const { timeIndicatorPosition, executeTrigger, play } = actionCommands;
const commandForProjectPortraits = ({ host, port, yHex }: { host: string, port: number, yHex : string }) => {
    ...
    const commandHex3 = transformToHexArray( play.commandAction);
